...
\usepackage{pmmeta}
\pmcanonicalname{MorseFunction}
\pmcreated{2013-03-22 13:53:15}
\pmmodified{2013-03-22 13:53:15}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{Morse function}
\pmrecord{8}{34632}
\pmprivacy{1}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Let $M$ be a smooth manifold.  A critical point of a map $u:M\to\R$ at $x\in M$ is called \PMlinkescapetext{{\em non-degenerate}} if the Hessian matrix $H_u$ (in any local coordinate system) at $x$ is non-degenerate.

A smooth function $u:M\to\R$ is called {\em Morse} if all its critical points are non-degenerate.  Morse functions exist on any smooth manifold, and in fact form an open \PMlinkname{dense}{Dense} subset of smooth functions on $M$ (this fact is often phrased ``a generic smooth function is Morse'').
%%%%%
%%%%%
\end{document}
