...
\usepackage{pmmeta}
\pmcanonicalname{TangentBundle}
\pmcreated{2013-03-22 13:58:59}
\pmmodified{2013-03-22 13:58:59}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{tangent bundle}
\pmrecord{5}{34756}
\pmprivacy{1}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Let $M$ be a differentiable manifold.  Let the tangent bundle $TM$ of $M$ be(as a set) the disjoint union $\coprod_{m\in M}T_mM$ of all the tangent spaces to $M$, i.e., the set of pairs $$\{(m,x)|m\in M, x\in T_mM\}.$$  This naturally has a manifold structure, given as follows.  For $M=\R^n$, $T\R^n$ is obviously isomorphic to $\R^{2n}$, and is thus obviously a manifold.  By the definition of a differentiable manifold, for any $m\in M$, there is a neighborhood $U$ of $m$ and a diffeomorphism $\vp:\R^n\to U$.  Since this map is a diffeomorphism, its derivative is an isomorphism at all points.  Thus $T\vp:T\R^n=\R^{2n}\to TU$ is bijective, which endows $TU$ with a natural structure of a differentiable manifold.  Since the transition maps for $M$ are differentiable, they are for $TM$ as well, and $TM$ is a differentiable manifold.  In fact, the projection $\pi:TM\to M$ forgetting the tangent vector and remembering the point, is a vector bundle.  A vector field on $M$ is simply a section of this bundle.  
